...
def test_ollama_loopback_url_allowed():
    ...
    assert validate_provider_base_url("https://127.0.0.1:11434/v1")
...
def test_get_authorized_runtime_ollama_does_not_treat_lookalike_host_as_loopback(monkeypatch):
    monkeypatch.setenv("AI_PLATFORM_PROVIDER", "ollama")
    monkeypatch.setenv("OPENAI_BASE_URL", "https://127.0.0.1.example.invalid/v1")
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    from api import dependencies
...
def test_get_authorized_runtime_ollama_allows_https_loopback_without_key(monkeypatch):
    monkeypatch.setenv("AI_PLATFORM_PROVIDER", "ollama")
    monkeypatch.setenv("OPENAI_BASE_URL", "https://127.0.0.1:11434/v1")
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    from api import dependencies
