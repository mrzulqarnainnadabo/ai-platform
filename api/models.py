...
class GenerateRequest(BaseModel):
    """Client selects a server-side model alias; never an upstream model/provider."""
    ...
    def to_platform_inputs(self, registry: ModelRegistry | None = None) -> tuple[List[Message], ModelConfig, ModelPolicy]:
        catalog = registry or ModelRegistry()
        policy = catalog.resolve(self.model_name)
        ...
        return parsed_messages, config, policy
